...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):        
        # print('A_path->>', self.A_paths[index])
        ...
        if self.opt.label_nc == 0:
            ...
            transform_A = get_transform(self.opt, params, method=Image.Resampling.NEAREST, normalize=False)
            A_tensor = transform_A(A) * 255.0
        # print('A_path->>', self.B_paths[index])
        B_path = self.B_paths[index]
        B = Image.open(B_path).convert('RGB')
        transform_B = get_transform(self.opt, params)      
        B_tensor = transform_B(B)
        # print('A_path->>', self.C_paths[index])
        ...
        with open(osp.join(pose_name), 'r') as f:
            ...
            try:
                ...
                pose_data = [0 for i in range(75)]
            pose_data = np.array(pose_data)
            # print('pose_data->>', pose_data.shape)
            pose_data = pose_data.reshape((-1,3))
            # print('pose_data.reshape((-1,3)->>', pose_data.shape)
        ...
        dense_mask = np.load(densepose_name)
